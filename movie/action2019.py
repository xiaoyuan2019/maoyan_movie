...
def parse_html(s):
    ...
    for item in movie_list:
        ...
        img = item.xpath('./div/a/div/img[2]/@data-src')[0]  # 绝对路径，电影图片url
        img_list.append(img)
...
def save_to_db(info):
    ...
    cursor.execute("insert into action2019 (title, score, img) \
...
def delete_table_data():
    conn = pymysql.connect(host='10.195.251.148', port=3306,
                           user='root', passwd='fhwsie', db='maoyan', charset='utf8')
    cursor = conn.cursor(cursor=pymysql.cursors.DictCursor)
    cursor.execute("delete from action2019;")
    conn.commit()
    cursor.close()
    conn.close()

def save_to_txt(content):
    with open('action2019.txt', 'a', encoding='utf-8') as f:
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        url = 'https://maoyan.com/films?showType=3&catId=5&yearId=14&offset={}'.format(i*30)
