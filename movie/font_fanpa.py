...
def findstar():
	...
	for i in range(len(subElementObj)):
		th1 = subElementObj[i].toprettyxml()
		rereobj = re.compile(r"name=\"(.*)\"")
		find_list = rereobj.findall(th1)
		# print(find_list)
		data.append(th1.replace(find_list[0], '').replace("\n", ''))
	# for item in data:
	# 	print('1:{}'.format(item))  #存储0-9共10个数字的坐标，因为坐标自始至终不会改变
	# 根据字体模板解码本次请求下载的字体
...
def web(url):
	...
	manary = soup.select(
		'body > div.banner > div > div.celeInfo-right.clearfix > div.movie-stats-container > div > div > span.stonefont')
	danwei = soup.select(
		'body > div.banner > div > div.celeInfo-right.clearfix > div.movie-stats-container > div > div > span.unit')
	star = soup.select(
	...
	data = "\'" + str(url) + "\', \'" + str(movie_name) + "\', \'" + str(ename_name) + "\', \'" + str(
		people_number) + "\', \'" + str(star_woff) + "\', \'" + str(getmanary) + "\'"
	# print(data)
	# to_mysql(data)
